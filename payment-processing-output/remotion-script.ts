...
// Auto-generated Remotion composition for avery-pay-platform
...
function IntroScene() {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: THEME.bg, justifyContent: "center", alignItems: "center" }}>
      <h1 style={{ color: THEME.fg, fontSize: 72, opacity }}>avery-pay-platform</h1>
      <p style={{ color: THEME.muted, fontSize: 28, opacity }}>PAI'D is **two systems in one repo**:</p>
    </AbsoluteFill>
  );
}
...
function AbstractionsScene() {
  const frame = useCurrentFrame();
  const items = ["go-backend/ (project_directory)","frontend/ (project_directory)","trust-fabric-frontend/ (project_directory)","archive/ (project_directory)","src/ (application_source)","wedge_portfolio/ (project_directory)"];
  return (
    <AbsoluteFill style={{ backgroundColor: THEME.bg, padding: 60 }}>
...
export function averypayplatformVideo() {
  return (
    <AbsoluteFill>
